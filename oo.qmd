...
format:
  dashboard:
    ...
    nav-buttons:
      - icon: github
        href: https://github.com/ohseyn/00
    css: style.css
---
...
회귀분석에 들어가기 앞서, 지역은 Ames의 **<span style="background-color: NavajoWhite;">Old_Town</span>**으로 한정한다. <br>

**다중 공선성**: 변수가 늘어남에 따라 변수간의 상관관계로 인해 통계적 오류가 발생.
...
<br>
신뢰구간에 0이 포함된다는 것은 해당 변수의 회귀계수(coef)가 0일 가능성을 배제할 수 없음을 의미한다. 
<br>
즉, <span style="background-color: NavajoWhite;">Longitude는 Old_Town의 Sale_Price에 영향을 미치지 않는다</span>라는 귀무가설을 기각할 수 없기에 분석에서 제외하였다.
...
**난방 품질**이 중요한 변수인 이유는 Old_Town의 주택들은 대부분 **오래되었기** 때문에, **난방 수준**이 신축 주택에 비해 **전체적으로 좋지 않을 것**으로 예상된다.<br>
그렇기에 **난방 품질**이 Old_Town에서 더욱 **높은 가치**를 가지게 되는 것이다.<br>

따라서, **<span style="background-color: NavajoWhite;">Ames Old_Town</span>의 집값을 가장 잘 설명할 수 있는 변수쌍은 <span style="background-color: NavajoWhite;">대지 면적과 난방품질</span>**다.
...
집이 **<span style="background-color: NavajoWhite;">최근에 지어질수록 집 가격도 올라가고</span>**, 5년 10년 단위로 묶어서 그래프를 부드럽게 만들 수 있습니다.<br>
